...
int main(int argc, char *argv[]) {
    int opt;
    char *imagen_a_ingresar = NULL;
    char *imagen_salida = NULL;
    int de_ayuda = 0;
    int de_jpeg = 0;
    int de_png = 0;

    while ((opt = getopt(argc, argv, "hi:o:j:p:")) != -1) {
        switch (opt) {
            case 'h':
                de_ayuda = 1;
                break;
            case 'i':
                imagen_a_ingresar = optarg;
                break;
            case 'o':
                imagen_salida= optarg;
                break;
            case 'j':
                de_jpeg = 1;
                break;
            case 'p':
                ...
                break;
            default:
                printf("Uso: %s -i <imagen_a_ingresar> -o <imagen_salida> -j -p\n", argv[0]);
                return 1;
        }
    }

    if (de_ayuda) {
        printf("Uso para PNG: %s -i <imagen_a_ingresar.png> -o <imagen_salida.png> -p -j\n", argv[0]);
        printf("Uso para JPG: %s -i <imagen_a_ingresar.jpg> -o <imagen_salida.jpg> -j -p\n", argv[0]);
        return 0;
    }

    if (de_jpeg==1) {
        invertJPEGColors(imagen_a_ingresar, imagen_salida);
        printf("Imagen JPEG invertida y guardada en %s\n", imagen_salida);
    } else if (de_png==1) {
        invertPNGColors(imagen_a_ingresar, imagen_salida);
        printf("Imagen PNG invertida y guardada en %s\n", imagen_salida);
    }
